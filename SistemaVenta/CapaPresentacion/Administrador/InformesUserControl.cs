...
namespace CapaPresentacion.Administrador
{
    public partial class InformesUserControl : UserControl
    {
        ...
        private void InformesUserControl_Load(object sender, EventArgs e)
        {
            var producto = new CN_Compra().ObtenerProductoMasVendido();

            if (producto != null)
            {
                txtProductoMasVendido.Text = producto.Nombre;
                txtStockProducto.Text = producto.Stock.ToString();
            }
            else
            {
                txtProductoMasVendido.Text = "Sin datos";
